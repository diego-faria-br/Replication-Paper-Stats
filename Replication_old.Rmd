...
Some visualization improvements were made in the graphic if compared to the paper. First, I associated different colors for information bellow, and above the cutoff, I also binned data is by its mean, as a point,  and confidence interval, as a line. I judge this approach as more adequate since it clearly shows how the data varies within each bin.
...
dat_g <- dat_filtered %>% group_by(bin) %>%summarise(mean_n = mean(nextyear,na.rm = T),mean_t = mean(turnout,na.rm = T),mean_e = mean(electorate,na.rm = T),n=n()) %>% mutate(x=seq(from=-0.1475,to=0.1475,by=0.005),won=ifelse(x>=0,1,0))
...
# g1<-ggplot()+geom_point(data=dat_g,aes(x=x,y=mean,fill=factor(won),size=n), shape=21)+
#    geom_smooth(aes(x=margin,y=nextyear,colour=factor(womanwon)),method = "lm",formula = y~poly(x,4),data = dat_filtered[dat_filtered$margin <= 0,], 
#               # color = "#F8766D", 
#               size = 1, se=TRUE) +
#   geom_smooth(aes(x=margin,y=nextyear,colour=factor(womanwon)),method = "lm",formula = y~poly(x,4),data = dat_filtered[dat_filtered$margin >= 0,], 
...
#   ggtitle("Women on Ballot Next Year in the Same District - Election 2") 
#   
#    g1 <- g1+ stat_summary_bin(binwidth = 0.005, size=.1, data = dat_filtered[dat_filtered$margin <= 0,],
#                          color = "#F8766D")+
#     stat_summary_bin(binwidth = 0.005, size=.1, data = dat_filtered[dat_filtered$margin >= 0,],
#                          color = "#00BFC4")+
#     theme_minimal()
...
#   
#   
#   g2 <- g2 + stat_summary_bin(fun="mean",binwidth = 0.005, size=.1, 
#                               data = dat_filtered[dat_filtered$margin <= 0,],
#                               color = "#F8766D")+
#       stat_summary_bin(fun="mean",binwidth = 0.005, size=.1, data = dat_filtered[dat_filtered$margin >= 0,], 
#                          color = "#00BFC4")+
#     theme_minimal()

g2 <- ggplot()+geom_point(data=dat_g,aes(x=x,y=mean_t,fill=factor(won),size=n), shape=21)+
...
g3 <- ggplot()+geom_point(data=dat_g,aes(x=x,y=mean_e,fill=factor(won),size=n), shape=20)+
...
The graphs present the same results as the paper. Visually it is possible to see that none of the graphs presents significant local average treatment effect in the around the threshold.
...
To test the robustness of the results, we do some falsification checks that are no present in the paper. The first one is robustness to specification, testing a linear model with different slopes. Also, we check for any covariate sorting around the threshold, especially for party affiliation, which could indicate some discontinuity of covariates.
...
```{r Graphics 3, hold=TRUE,warning=FALSE,error=FALSE,message=FALSE,fig.width=10}
...
According to a research carried out by [@pew], from Whashigton DC. Democrats and Republicans have different views about gender gaps. Some findinds are that: 
...
So, the hyphotesis to be testest here is about partisanship would affect the results. In order to test this hypothesis the elections were divided by  party afiliation of the candidate. The hypothesis is that electing a womand would have an higher impact on Republicans candidates, since the pre-conceptions about gender roles for this group would be challenged.
...
stargazer(reg1d,reg2d,reg3d,reg1p,reg2p,reg3p,type = "html",title= "Table 4 - Partisanship",
          keep.stat=c("n","rsq","bic"), 
